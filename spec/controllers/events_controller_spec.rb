  ...
  describe "GET index" do
    ...
    context "when testing index functionality of tags" do
      ...
      it "assigns @events when query has no tags" do
        get :index
        expect(assigns(:events)).to eq([test_event,test_event_2,test_event_3])
      end
      
      it "assigns @events when query tags = dog" do
        get :index, tags: ['dog'] # <--- tbc query parameters for get request
        expect(assigns(:events)).to eq([test_event,test_event_3])
      end
    ...
    context "when testing index functionality of ngos" do
      ...
      let!(:charity) { Ngo.create!(name: 'charityRus', email: 'c@gmail.com', password: 'xzyabc123') } 
